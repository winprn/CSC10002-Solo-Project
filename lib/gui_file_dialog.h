...
GuiFileDialogState InitGuiFileDialog(const char* initPath) {
  ...
  // Init window data
  state.windowBounds = (Rectangle){(float)(GetScreenWidth() / 2),
                                   (float)(GetScreenHeight() / 2), 440, 310};
  ...
  if (state->windowActive) {
      ...
      if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        ...
        if (CheckCollisionPointRec(
                ...
                (Rectangle){state->windowBounds.x, state->windowBounds.y,
                            (float)state->windowBounds.width, 24})) {
          state->dragMode = true;
...
#else
    state->filesListActive = GuiListViewEx(
        (Rectangle){state->windowBounds.x + 8, state->windowBounds.y + 48 + 20,
                    state->windowBounds.width - 16,
                    state->windowBounds.height - 60 - 16 - 68},
        (const char**)dirFilesIcon, state->dirFiles.count, &state->itemFocused,
        &state->filesListScrollIndex, state->filesListActive);
#endif
    ...
    state->fileTypeActive =
        GuiComboBox((Rectangle){state->windowBounds.x + 72,
                                state->windowBounds.y +
                                    state->windowBounds.height - 24 - 12,
                                state->windowBounds.width - 184, 24},
