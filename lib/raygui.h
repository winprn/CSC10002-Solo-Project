...
RAYGUIAPI bool GuiButton(
    ...
    bool isOnlyIcon = false);  // Button control, returns true when clicked
...
RAYGUIAPI void
GuiSetIconScale(unsigned int scale);  // Set icon scale (1 by default)
...
bool GuiButton(Rectangle bounds, const char* text, bool isIconOnly) {
  GuiState state = guiState;
  bool pressed = false;
  if (!isIconOnly) {
    Vector2 size = MeasureTextEx(Settings::font_regular, text, 20, 1);
    bounds.width = fmax(bounds.width, size.x + 10);
    bounds.height = fmax(bounds.height, size.y + 10);
  }
      ...
      if (fontDataSize > 0) {
        ...
        if (fontImageCompSize < fontImageUncompSize) {
          ...
          if (dataUncompSize != fontImageUncompSize)
            RAYGUI_LOG("WARNING: Uncompressed font atlas image data could be "
                       "corrupted");
...
static void GuiDrawRectangle(Rectangle rec, int borderWidth, Color borderColor,
  ...
  DrawRectangleRounded(rec, 0.12, 20, color);
}
