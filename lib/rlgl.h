...
#define RLAPI                                                                  \
  __declspec(                                                                  \
      dllexport)  // We are building the library as a Win32 shared library (.dll)
...
#if defined(GRAPHICS_API_OPENGL_11)
RLAPI void
rlEnableStatePointer(int vertexAttribType,
...
RLAPI void
rlglClose(void);  // De-inititialize rlgl (buffers, shaders, textures)
...
RLAPI void rlComputeShaderDispatch(
    ...
    unsigned int
        groupZ);  // Dispatch compute shader (equivalent to *draw* for graphics pilepine)

// Shader buffer storage object management (ssbo)
RLAPI unsigned int
rlLoadShaderBuffer(unsigned long long size, const void* data,
...
RLAPI void rlUpdateShaderBufferElements(
    unsigned int id, const void* data, unsigned long long dataSize,
    unsigned long long offset);  // Update SSBO buffer data
RLAPI unsigned long long
rlGetShaderBufferSize(unsigned int id);  // Get SSBO buffer size
RLAPI void
rlReadShaderBufferElements(unsigned int id, void* dest,
                           unsigned long long count,
                           unsigned long long offset);  // Bind SSBO buffer
RLAPI void rlBindShaderBuffer(unsigned int id,
                              unsigned int index);  // Copy SSBO buffer data

// Buffer management
RLAPI void
rlCopyBuffersElements(unsigned int destId, unsigned int srcId,
                      unsigned long long destOffset,
                      unsigned long long srcOffset,
                      unsigned long long count);  // Copy SSBO buffer data
RLAPI void rlBindImageTexture(unsigned int id, unsigned int index,
                              unsigned int format,
...
typedef struct rlglData {
  ...
  struct {
    ...
    bool
        texDepth;  // Depth textures supported (GL_ARB_depth_texture, GL_WEBGL_depth_texture, GL_OES_depth_texture)
    bool
...
#if defined(GRAPHICS_API_OPENGL_11)
static int
rlGenTextureMipmapsData(unsigned char* data, int baseWidth,
                        int baseHeight);  // Generate mipmaps data on CPU side
static unsigned char*
rlGenNextMipmapData(unsigned char* srcData, int srcWidth,
...
void rlDrawVertexArrayElements(int offset, int count, const void* buffer) {
  glDrawElements(GL_TRIANGLES, count, GL_UNSIGNED_SHORT,
                 (const unsigned short*)buffer + offset);
}
...
void rlDrawVertexArrayElementsInstanced(int offset, int count,
  ...
  glDrawElementsInstanced(GL_TRIANGLES, count, GL_UNSIGNED_SHORT,
                          (const unsigned short*)buffer + offset, instances);
#endif
}
...
int rlGetLocationUniform(unsigned int shaderId, const char* uniformName) {
  ...
  if (location == -1)
    TRACELOG(RL_LOG_WARNING,
             "SHADER: [ID %i] Failed to find shader uniform: %s", shaderId,
             uniformName);
  else
    TRACELOG(RL_LOG_INFO,
...
int rlGetLocationAttrib(unsigned int shaderId, const char* attribName) {
  ...
  if (location == -1)
    TRACELOG(RL_LOG_WARNING,
             "SHADER: [ID %i] Failed to find shader attribute: %s", shaderId,
             attribName);
  else
    TRACELOG(RL_LOG_INFO,
...
unsigned int rlLoadShaderBuffer(unsigned long long size, const void* data,
                                int usageHint) {
  unsigned int ssbo = 0;
...
void rlUpdateShaderBufferElements(unsigned int id, const void* data,
...
void rlReadShaderBufferElements(unsigned int id, void* dest,
                                unsigned long long count,
                                unsigned long long offset) {
#if defined(GRAPHICS_API_OPENGL_43)
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, id);
...
void rlCopyBuffersElements(unsigned int destId, unsigned int srcId,
                           unsigned long long destOffset,
                           unsigned long long srcOffset,
                           unsigned long long count) {
#if defined(GRAPHICS_API_OPENGL_43)
...
// Bind image texture
void rlBindImageTexture(unsigned int id, unsigned int index,
                        unsigned int format, int readonly) {
#if defined(GRAPHICS_API_OPENGL_43)
...
static unsigned char* rlGenNextMipmapData(unsigned char* srcData, int srcWidth,
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width; x++) {
      ...
      prow[0] = (srcData[(y2 * srcWidth + x2) * 4 + 0] +
                 srcData[(y2 * srcWidth + x2 + 1) * 4 + 0]) /
                2;
      prow[1] = (srcData[(y2 * srcWidth + x2) * 4 + 1] +
                 srcData[(y2 * srcWidth + x2 + 1) * 4 + 1]) /
                2;
      prow[2] = (srcData[(y2 * srcWidth + x2) * 4 + 2] +
                 srcData[(y2 * srcWidth + x2 + 1) * 4 + 2]) /
                2;
      prow[3] = (srcData[(y2 * srcWidth + x2) * 4 + 3] +
                 srcData[(y2 * srcWidth + x2 + 1) * 4 + 3]) /
                2;
